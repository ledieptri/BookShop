			...
			<div class="account">
				<div class="account__left">
					...
					<p class="icon" data-choice="2">
						<i class="fab fa-elementor"></i> <span> Quản lý đơn hàng </span>
					</p>
					<a href="<c:url value='/logout?action=logout'/>" class="icon"
						data-choice="3"> <i class="fas fa-power-off"></i> Đăng xuất
					</a>
				</div>
						...
						<form class="account-form show" data-form="1" id="form-profile"
							...
							<div class="form-group">
								<label for="fullName">Họ và tên</label> <input type="text"
									name="fullName" id="fullName" value="${USERMODEL.fullName}"
									required="required">
							</div>
							<div class="form-group">
								<label for="email">Email</label> <input type="email"
									name="email" id="email" value="${USERMODEL.email}"
									required="required" readonly="readonly">
							</div>
							<div class="form-group">
								<label for="phone">Số điện thoại</label> <input type="text"
									name="phone" id="phone" value="${USERMODEL.phone}"
									required="required">
							</div>
							<div class="form-group">
								<label for="address">Địa chỉ giao hàng</label> <input
									type="text" name="address" id="address"
									value="${USERMODEL.address}" required="required">
							</div>
							<div class="form-group">
								<button type="button" name="btnUpdateUser" id="btnUpdateUser">Cập
									nhật</button>
							</div>
									...
									<c:forEach var="order" items="${listOrder}">
										<tr>
											...
											<td><c:forEach var="orderDetail"
													...
													<c:if
														test="${orderDetail.orderId.orderId == order.orderId && order.userId.userId == USERMODEL.userId}">	                                        		                                       			
