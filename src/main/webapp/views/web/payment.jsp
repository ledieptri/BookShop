		...
		<div class="content__container">
			<div class="payment__content">
				<form onsubmit="buttonName.disabled=true; return true;"
					action='<c:url value="/payment"></c:url>' method="post"
					class="payment__content-left">
						...
						<div class="payment-form-address">
							<div class="address">
								<div class="address-item">
									<label for="address-name">Họ và tên</label> <input type="text"
										id="address-name" name="fullName"
										value='<c:out value="${USERMODEL.fullName}"></c:out>' required>
								</div>
								<div class="address-item">
									<label for="address-phone">Số điện thoại</label> <input
										type="text" id="address-phone" name="phone"
										value='<c:out value="${USERMODEL.phone}"></c:out>' required>
								</div>
								<div class="address-item">
									<label for="address-email">Địa chỉ email</label> <input
										type="email" id="address-email" name="email"
										value='<c:out value="${USERMODEL.email}"></c:out>' required>
								</div>
								<div class="address-item">
									<label for="address-inf">Địa chỉ nhận hàng</label> <input
										type="text" id="address-inf" name="address"
						...
						<div class="payment-form-order-group">
							<c:forEach var="item" items="${cart.cartDetails}">
								<div class="payment-form-order-item">
									<span class="product-item"> <span>x${item.quantity}</span>
										<a href="#">${item.product.productName}</a>
									</span> <span class="product-price"> <c:set var="p"
											value="${item.getTotal()}" /> <fmt:setLocale value="vi_VN" />
						...
						<div class="payment-form-order-foot">
							<div class="order-totals">
								<div class="order-totals-line">
									<p>Tạm tính :</p>
									<span> <c:set var="p" value="${totalSum}" /> <fmt:setLocale
