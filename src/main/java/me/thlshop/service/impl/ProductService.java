...
public class ProductService implements IProductService {
	...
	@Override
	public List<Integer> findPromotionProductIdList() {
		List<ProductEntity> promotionProducts = productDAO.findPromotionPercentProduct();
		List<Integer> promotionIds = new ArrayList<>();
		if (promotionProducts != null && !promotionProducts.isEmpty()) {
			promotionIds = promotionProducts.stream().map(product -> product.getProductId()).toList();
		}
