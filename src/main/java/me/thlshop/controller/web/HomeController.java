...
import me.thlshop.promotion.GiamGiaPhanTram;
import me.thlshop.promotion.GiamGiaService;
...
public class HomeController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		List<Integer> promotionIdProductList = productService.findPromotionProductIdList();
		GiamGiaService giamGiaService = new GiamGiaPhanTram(promotionIdProductList);
		List<ProductEntity> productList = productService.findProductOutstanding();
		if (productList != null && !productList.isEmpty()) {
			productList.forEach(product -> {
				giamGiaService.xuLyGiamGia(product);
			});
		}
		...
		if (action != null && action.equals("login")) {
			...
			if (userModel != null) {
				...
				response.sendRedirect(request.getContextPath()
						+ "/dang-nhap?action=login&message=user_password_invalid&alert=danger");
			}
		}
	}
}
